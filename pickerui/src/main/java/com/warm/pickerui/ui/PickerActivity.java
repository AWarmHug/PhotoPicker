...
import com.warm.picker.pick.filter.ImageFilter;
import com.warm.picker.pick.work.AlbumFinder;
import com.warm.picker.pick.work.ImageFinder;
...
public class PickerActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void setAlbumUi(List<Album> albums, boolean restore) {
        ...
        for (int i = 0; i < albums.size(); i++) {
            if (albums.get(i).getBucketId().equals(Album.BUCKET_ID_ALL)) {
                tvAlbum.setEnabled(albums.get(i).getCount() != 0);
                break;
            }
        }
        if (!restore) {
